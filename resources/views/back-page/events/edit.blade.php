<x-backPage.layout>
    ...
    <form id="form" data-form="validate" action="{{ route('events.update', $event->slug) }}" method="post" class="bg-white p-6" enctype="multipart/form-data">
        ...
        <div class="grid gap-4 mb-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <select id="category_id" name="category_id" class="rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required data-parsley-trigger="change" required>
                    ...
                    <option {{ $event->category_id == $id ? 'selected' : '' }} value="{{ $id }}">{{ $category }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="mb-4">
            ...
            <input onchange="previewImage()" type="file" name="images" id="images" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" data-parsley-trigger="keyup" value="{{ old('images') }}">
            @if($event->images)
            <img src="{{ $event->images }}" class="rounded mt-2 text-sm text-gray-500 dark:text-gray-300 img-preview max-w-96">
            @else
        ...
        <div class="mb-4">
            ...
            <select required data-parsley-errors-container="#selec2-error" id="speakers" name="speakers[]" multiple="multiple" class="block w-full mt-1 rounded-md shadow-sm border-gray-300">
                ...
                <option  {{ in_array($id, $speakerIds) ? 'selected' : '' }} value="{{ $id }}">{{ $speaker }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select id="status" name="status" class="rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required data-parsley-trigger="change" required>
                <option value="">Choose your event speakers</option>
                <option {{ $event->status ? 'selected' : '' }} value="1">Published</option>
                <option {{ $event->status ? '' : 'selected' }} value="0">Draft</option>
            </select>
        </div>
